...
/*
 * Note on the template: this wouldn't be required if only C++ supported covariant retur types in
 * smart pointers... :(
 */
template <ConcreteMedium MediumType> class JsonParsingLib {

    MediaSerializationConfigs configs;

  public:
    explicit JsonParsingLib(MediaSerializationConfigs newConfigs)
    ...
    [[nodiscard]] auto optionalValidatedFieldParser(
        const QJsonObject &obj, const QString &key,
        std::function<ValidatedField<FieldType> &(MediumType &)> getField) const
        -> std::function<std::optional<JsonDeserializationError>(MediumType &)> {

        return [this, obj, key,
                getField](MediumType &medium) -> std::optional<JsonDeserializationError> {
            if (!obj.contains(key)) {
                return std::nullopt;
            }
        ...
        return [this, obj, key,
            ...
            for (const auto &item : jsonValue.toArray()) {
                if (!item.isString()) {
                    return JsonDeserializationError{
                        .code = JsonDeserializationError::Code::WrongMediumFieldType,
                        .errorLocation = obj,
                        .message = QString("An item in the array for field '%1' is not a string.")
                                       .arg(key)};
                }
            ...
            if (!getField(medium).set(values)) {
                return JsonDeserializationError{
                    ...
                    .errorLocation = obj,
                    .message =
                        QString("Values for field '%1' failed semantic validation.").arg(key)};
            }
    ...
    [[nodiscard]] auto deserializeCommonFields(const QJsonObject &mediumObject,
        ...
        std::variant<JsonDeserializationError, std::unique_ptr<MediumType>> result =
            std::move(medium);

        result = andThen(std::move(result), optionalValidatedSetParser(mediumObject, "authors",
                                                                       [](MediumType &m) -> auto & {
                                                                           return m.authors();
                                                                       }));
        result =
            andThen(std::move(result), optionalValidatedFieldParser<QString>(
                                           mediumObject, "language",
                                           [](MediumType &m) -> auto & { return m.language(); }));
        result =
            andThen(std::move(result),
                    optionalValidatedSetParser(mediumObject, "topics", [](MediumType &m) -> auto & {
                        return m.userData().topics();
                    }));
        result = andThen(std::move(result), optionalValidatedFieldParser<QString>(
                                                mediumObject, "notes", [](MediumType &m) -> auto & {
                                                    return m.userData().notes();
                                                }));
        result = andThen(std::move(result),
                         optionalValidatedFieldParser<MediumUserData::PriorityLevel>(
